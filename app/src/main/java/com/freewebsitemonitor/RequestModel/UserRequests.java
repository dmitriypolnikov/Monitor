...
//                                        .setRetryStrategy(RetryStrategy.DEFAULT_EXPONENTIAL)
    ...
    public void getStatusPing(MyApp boloTalk, String monitorID, String monitorName, String address, String keywords, String port, String type, String interval, String startDate, String previousStatus) {

        String key = "Uqj7RG6WqNfCqxvht8MhnkFSv";
        String sig = "f3bd2eda5604b98fdf287f55e789f7ec";
        String hash = boloTalk.getKey(USER_HASH);

        Call<RootMonitorStatus> book = boloTalk.getRetrofitService().getStatusPing(sig, key, hash, monitorID);
        book.enqueue(new Callback<RootMonitorStatus>() {
            @Override
            public void onResponse(Call<RootMonitorStatus> call, Response<RootMonitorStatus> response) {
                ...
                if (errorType == 0) {
                    ...
                    boolean isGet = MonitorUtil.isHttpConnection(address, type, keywords);
                    if (isGet) {
                        funSendStatus(monitorID, boloTalk, "1", mobileDateTime, deviceName, ipAddress, previousStatus, monitorName, address, keywords, port, type, interval, startDate);
                    } else {
                        funSendStatus(monitorID, boloTalk, "2", mobileDateTime, deviceName, ipAddress, previousStatus, monitorName, address, keywords, port, type, interval, startDate);
                    }
                }
            }
